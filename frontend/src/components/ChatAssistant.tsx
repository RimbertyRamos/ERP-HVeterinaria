...
export const ChatAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'bot', content: '¡Hola! Soy VET-AI. Estoy aquí para ayudarte con emergencias veterinarias. ¿Qué sucede con tu mascota?' }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const replyText = response?.reply || 'Lo siento, no recibí una respuesta válida de la IA.';
      setMessages(prev => [...prev, { role: 'bot', content: String(replyText) }]);
    } catch (err) {
      setMessages(prev => [...prev, { role: 'bot', content: '⚠️ Error: No pude conectarme con el servidor de inteligencia artificial.' }]);
    } finally {
      setLoading(false);
    }
  };
        ...
        {isOpen && (
          ...
          >
            <header className="bg-gradient-to-r from-primary to-emerald-500 p-6 text-slate-900">
               <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        <h3 className="font-black text-sm uppercase tracking-tighter">Asistente VET-AI</h3>
                        <p className="text-[10px] font-bold opacity-80 uppercase">Emergencias 24/7</p>
                     </div>
                  </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50 dark:bg-slate-950/50">
               {messages.map((msg, i) => (
                  <div key={`msg-${i}`} className={cn("flex", msg.role === 'user' ? "justify-end" : "justify-start")}>
                     <div className={cn(
                        ...
                        {String(msg?.content || '').split('\n').map((line, j) => (
                           <p key={j} className={line.startsWith('⚠️') ? "text-amber-500 font-bold mb-1" : ""}>{line}</p>
                        ))}
                     </div>
                  </div>
               ))}
