...
const buildLineasFicha = (ficha: FichaPendiente) => {
  ...
  lines.push({ tipo: 'SERVICIO', desc: ficha.servicio.nombre, subtotal: Number(ficha.servicio.precio_base) });
  for (const o of ficha.ordenes_lab)
    lines.push({ tipo: 'LABORATORIO', desc: o.examen.nombre, subtotal: Number(o.examen.precio) });
  for (const c of ficha.consumos ?? [])
    lines.push({ tipo: 'SUMINISTRO', desc: `${c.producto.nombre} x${c.cantidad}`, subtotal: Number(c.producto.precio_venta) * c.cantidad });
  for (const d of ficha.soap?.receta?.detalles ?? [])
    lines.push({ tipo: 'FARMACIA', desc: `${d.producto.nombre} x${d.cantidad}`, subtotal: Number(d.producto.precio_venta) * d.cantidad });
  return lines;
};
...
export const POS: React.FC = () => {
  ...
  const [selectedFicha, setSelectedFicha] = useState<FichaPendiente | null>(null);
  ...
  useEffect(() => { 
    ...
    else if (tab === 'VENTA_DIRECTA' && productos.length === 0) fetchProductos();
    else if (tab === 'HISTORIAL') fetchRecibos();
  }, [tab, fetchPendientes, fetchProductos, fetchRecibos, productos.length]);

  // Cálculos totales
  const totalCart = cart.reduce((acc, item) => acc + Number(item.producto.precio_venta) * item.cantidad, 0);
  const total = tab === 'CONSULTAS' ? (selectedFicha ? calcTotalFicha(selectedFicha) : 0) : totalCart;
    ...
    try {
      ...
      if (tab === 'CONSULTAS') {
        ...
        res = await api.ventaDirecta({
          ...
          metodo_pago: metodo,
          monto_recibido: total,
          productos: cart.map(c => ({ id: c.producto.id, cantidad: c.cantidad }))
        });
      }
  ...
  const printRecibo = (recibo: ReciboCaja) => {
    ...
    const lineas = (recibo.detalles ?? [])
      .map((d) => `<tr><td>${d.tipo}</td><td>${d.descripcion}</td><td style="text-align:right;">${d.cantidad}</td><td style="text-align:right;">Bs.${Number(d.subtotal).toFixed(2)}</td></tr>`)
      .join('');
    const html = `
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex h-full overflow-hidden">
      ...
      <section className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-950 overflow-y-auto p-8">
        <header className="mb-6">
          <div className="flex justify-between items-end">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Caja Central</h2>
            <div className="flex bg-white dark:bg-slate-900 p-1 rounded-lg border border-slate-200 dark:border-slate-800">
              <button
                onClick={() => setTab('CONSULTAS')} 
                className={cn('px-4 py-1.5 text-sm font-bold rounded-md transition-colors', tab === 'CONSULTAS' ? 'bg-primary text-slate-900' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300')}
              >Cobrar Consultas</button>
              <button 
                onClick={() => setTab('VENTA_DIRECTA')} 
                className={cn('px-4 py-1.5 text-sm font-bold rounded-md transition-colors', tab === 'VENTA_DIRECTA' ? 'bg-primary text-slate-900' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300')}
              >Venta Directa</button>
              <button
                onClick={() => setTab('HISTORIAL')}
                className={cn('px-4 py-1.5 text-sm font-bold rounded-md transition-colors', tab === 'HISTORIAL' ? 'bg-primary text-slate-900' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300')}
              >Historial</button>
            </div>
          </div>
          ...
          <div className="mt-4 relative">
            ...
            <input
              ...
              placeholder={tab === 'CONSULTAS' ? "Buscar por paciente, dueño o código..." : tab === 'VENTA_DIRECTA' ? "Buscar producto..." : "Buscar por nro recibo, cliente o cajero..."}
              value={search}
        ...
        {exito && (
          <div className="mb-4 flex items-center gap-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 px-5 py-4 text-emerald-700 dark:text-emerald-400">
            ...
            <span className="font-bold">Transacción exitosa — Recibo {exito} generado</span>
            {lastRecibo && (
              <button onClick={() => printRecibo(lastRecibo)} className="ml-2 rounded-lg bg-emerald-600 text-white px-3 py-1.5 text-xs font-bold flex items-center gap-1">
                <Icons.Printer size={14} /> Imprimir
              </button>
            )}
            <button onClick={() => setExito(null)} className="ml-auto"><Icons.X size={16} /></button>
          </div>
        )}
        ...
        {tab === 'CONSULTAS' ? (
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Fichas Pendientes de Cobro {fichas.length > 0 && `(${fichas.length})`}</h3>
            {loadingFichas ? (
              [...Array(3)].map((_, i) => <div key={i} className="h-28 rounded-xl bg-white dark:bg-slate-900 animate-pulse" />)
              ...
              fichasFiltradas.map((f) => {
                ...
                return (
                  <div key={f.id} onClick={() => handleSeleccionarFicha(f)} className={cn('bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border-2 transition-all cursor-pointer', isActive ? 'border-primary shadow-primary/20 shadow-md' : 'border-transparent hover:border-primary/30')}>
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <div className="size-9 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-amber-600 dark:text-amber-400"><Icons.Patients size={18} /></div>
                        <div>
                          <h4 className="font-bold text-slate-900 dark:text-white">{f.mascota.nombre}</h4>
                          <p className="text-xs text-slate-500">Dueño: {f.mascota.propietario.nombre}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-xs text-slate-400 font-mono">{f.cod_ficha}</span>
                        <p className="text-lg font-extrabold text-slate-900 dark:text-white">Bs.{t.toFixed(2)}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      <span className="bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[10px] font-bold px-2 py-0.5 rounded uppercase">{f.servicio.nombre}</span>
                    </div>
                  </div>
                );
              })
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {loadingProductos ? (
               [...Array(8)].map((_, i) => <div key={i} className="h-32 rounded-xl bg-white dark:bg-slate-900 animate-pulse" />)
            ) : (
              productosFiltrados.map(p => (
                <div key={p.id} onClick={() => handleAddToCart(p)} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 cursor-pointer hover:border-primary/50 hover:shadow-md transition-all flex flex-col h-full text-center group relative overflow-hidden">
                   {p.stock_actual <= 0 && <div className="absolute inset-0 bg-white/60 dark:bg-slate-900/60 backdrop-blur-[1px] flex items-center justify-center z-10"><span className="bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded">AGOTADO</span></div>}
                   <div className="text-3xl mb-2 group-hover:scale-110 transition-transform">💊</div>
                   <h4 className="text-sm font-bold text-slate-900 dark:text-white flex-1">{p.nombre}</h4>
                   <p className="text-xs text-slate-500 font-mono my-1">{p.categoria?.nombre || 'General'}</p>
                   <div className="mt-auto flex justify-between items-center pt-2 border-t border-slate-100 dark:border-slate-800">
                      <span className="text-xs font-medium text-slate-400">Stock: {p.stock_actual}</span>
                      <span className="text-sm font-extrabold text-primary">Bs.{Number(p.precio_venta).toFixed(2)}</span>
                   </div>
                </div>
              ))
            )}
          ...
          <div className="space-y-4">
            ...
            {loadingRecibos ? (
              [...Array(3)].map((_, i) => <div key={i} className="h-24 rounded-xl bg-white dark:bg-slate-900 animate-pulse" />)
            ) : recibosFiltrados.length === 0 ? (
              <div className="py-20 text-center text-slate-400">No hay pagos para ese filtro.</div>
            ) : (
              recibosFiltrados
                .filter((r) => {
                  const q = search.toLowerCase();
                ...
                .map((r) => (
                  <div key={r.id} className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-bold text-slate-900 dark:text-white">{r.num_recibo}</p>
                        <p className="text-xs text-slate-500">{new Date(r.fecha_pago).toLocaleString()} · Cajero: {r.cajero?.nombre}</p>
                        {r.nombre_cliente && <p className="text-xs text-slate-500">Cliente: {r.nombre_cliente}</p>}
                      </div>
                      <div className="text-right">
                        <p className="text-lg font-extrabold">Bs.{Number(r.total).toFixed(2)}</p>
                        <button onClick={() => printRecibo(r)} className="mt-1 text-xs font-bold text-primary flex items-center gap-1">
      ...
      <aside className="w-96 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 flex flex-col shadow-xl z-10">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Detalle del Cobro</h2>
          <p className="text-xs text-slate-500 mt-1">
            {tab === 'CONSULTAS' ? 
               (selectedFicha ? `${selectedFicha.cod_ficha} — ${selectedFicha.mascota.nombre}` : 'Seleccione una ficha') : 
          ...
          {tab === 'CONSULTAS' ? (
            !selectedFicha ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
                ...
                <p className="text-sm font-medium text-center">Selecciona una ficha<br />de la lista para cobrar</p>
              </div>
            ) : (
              buildLineasFicha(selectedFicha).map((l, i) => (
                <div key={i} className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-slate-800">
                  <div>
                    <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded mr-2 uppercase', l.tipo === 'SERVICIO' && 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400', l.tipo === 'LAB' && 'bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400', l.tipo === 'FARMACIA' && 'bg-teal-50 text-teal-600 dark:bg-teal-900/30 dark:text-teal-400')}>{l.tipo}</span>
                    <span className="text-sm text-slate-700 dark:text-slate-300">{l.desc}</span>
                  </div>
                  <span className="text-sm font-bold text-slate-900 dark:text-white">Bs.{l.subtotal.toFixed(2)}</span>
                </div>
              ))
            )
          ) : (
            cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
                <Icons.POS size={48} className="opacity-20" />
                <p className="text-sm font-medium text-center">Agrega productos<br />haciendo clic en ellos</p>
              </div>
            ) : (
              cart.map((item, i) => (
                <div key={i} className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-slate-800 group">
                  <div className="flex-1">
                    <div className="flex justify-between">
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-300 line-clamp-1 pr-2">{item.producto.nombre}</span>
                      <button onClick={() => handleRemoveFromCart(item.producto.id)} className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Icons.X size={14}/></button>
                    </div>
                    <div className="flex justify-between mt-1">
                      <span className="text-[10px] font-mono text-slate-500">{item.cantidad} x Bs.{Number(item.producto.precio_venta).toFixed(2)}</span>
                      <span className="text-sm font-bold text-slate-900 dark:text-white">Bs.{(item.cantidad * Number(item.producto.precio_venta)).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              ))
        ...
        {((tab === 'CONSULTAS' && selectedFicha) || (tab === 'VENTA_DIRECTA' && cart.length > 0)) && (
          <div className="p-6 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 space-y-4">
            ...
            <div className="flex justify-between items-end py-2 border-b border-dashed border-slate-200 dark:border-slate-700">
              ...
              <span className="text-2xl font-extrabold text-slate-900 dark:text-white">Bs.{total.toFixed(2)}</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['EFECTIVO', 'TARJETA', 'QR'] as MetodoPago[]).map((m) => (
                ...
                >
                  ...
                  {m === 'TARJETA' && <Icons.CreditCard size={18} className="mb-1" />}
