...
const tipoColor = (tipo_item?: string) => {
  if (tipo_item === 'MEDICAMENTO') return 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400';
  if (tipo_item === 'VACUNA') return 'bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400';
  if (tipo_item === 'INSUMO_MEDICO') return 'bg-teal-50 text-teal-600 dark:bg-teal-900/30 dark:text-teal-400';
  if (tipo_item === 'ALIMENTO') return 'bg-orange-50 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400';
  return 'bg-slate-50 text-slate-600 dark:bg-slate-800 dark:text-slate-400';
};

const inputCls = 'w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary text-slate-900 dark:text-slate-100';
const labelCls = 'block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1';

export const Inventory: React.FC = () => {
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const initialSection = user?.rol?.nombre === 'FARMACEUTICO' ? 'FARMACIA' : 'PRODUCTOS';
  const [section, setSection] = useState<'PRODUCTOS' | 'FARMACIA'>(initialSection);
  ...
  const [stockForm, setStockForm] = useState<StockForm>({ tipo: 'INGRESO', cantidad: '', motivo: '' });
  ...
  const categoriasNombres = ['Todo', ...Array.from(new Set(productos.map((p) => p.categoria.nombre)))];
  ...
  const valorInventario = productos.reduce((s, p) => s + Number(p.precio_venta) * p.stock_actual, 0);
  ...
  const setField = (f: Partial<ProductoForm>) => setForm((prev) => ({ ...prev, ...f }));
  ...
  const handleAjustarStock = async () => {
    ...
    if (!cant || cant <= 0) { setError('Ingrese una cantidad válida mayor a 0'); return; }
    if (stockForm.tipo === 'SALIDA' && selected && cant > selected.stock_actual) {
      setError(`Stock insuficiente. Disponible: ${selected.stock_actual} unidades`);
      return;
    }
    setSaving(true);
    setError('');
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Inventario y Farmacia</h2>
          <p className="text-slate-500 dark:text-slate-400">Gestión de stock y suministros</p>
        </div>
        <div className="flex gap-3">
          <button
            className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-red-100 dark:bg-red-900/30 text-red-600 rounded-lg"><Icons.AlertTriangle size={18} /></div>
            {bajoStock > 0 && <span className="text-xs font-bold text-red-600 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded">Atención</span>}
          </div>
          <p className="text-slate-500 text-sm font-medium">Bajo Stock</p>
          <p className="text-2xl font-bold mt-1">{loading ? '—' : `${bajoStock} Producto${bajoStock !== 1 ? 's' : ''}`}</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-primary/20 text-primary rounded-lg"><Icons.POS size={18} /></div>
          </div>
          <p className="text-slate-500 text-sm font-medium">Valor Inventario</p>
          <p className="text-2xl font-bold mt-1">{loading ? '—' : `Bs.${valorInventario.toFixed(0)}`}</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-lg"><Icons.FileText size={18} /></div>
          </div>
          <p className="text-slate-500 text-sm font-medium">Total Productos</p>
          <p className="text-2xl font-bold mt-1">{loading ? '—' : productos.length}</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-lg"><Icons.History size={18} /></div>
          </div>
          <p className="text-slate-500 text-sm font-medium">Categorías</p>
          <p className="text-2xl font-bold mt-1">{loading ? '—' : categorias.length}</p>
        </div>
      </div>
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-6 py-4">
                          <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{p.nombre}</span>
                          {p.descripcion && <p className="text-[10px] text-slate-400 mt-0.5">{p.descripcion}</p>}
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span className={cn('px-2 py-1 text-[10px] font-bold rounded uppercase', tipoColor(p.categoria.tipo_item))}>
                            {tipoLabel(p.categoria.tipo_item)}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className={cn('text-sm font-bold', critico ? 'text-red-600' : 'text-slate-900 dark:text-slate-100')}>
                              {p.stock_actual}
                            </span>
                            <div className="w-14 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                              <div
                                className={cn('h-full rounded-full', critico ? 'bg-red-500' : 'bg-primary')}
                                style={{ width: `${Math.min((p.stock_actual / Math.max(p.stock_minimo * 2, 1)) * 100, 100)}%` }}
                              />
                            </div>
                            {critico && <Icons.AlertTriangle size={12} className="text-red-500" />}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm font-medium text-slate-500">{p.stock_minimo}</td>
      ...
      <AnimatePresence>
          ...
          <ModalOverlay onClose={closeModal}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className={labelCls}>{modal === 'create' ? 'Stock Inicial' : 'Stock Actual'}</label>
                  <input
                    type="number"
                    min="0"
                    className={inputCls}
            ...
            {error && <p className="mt-4 text-sm text-red-600 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg">{error}</p>}

            <div className="flex gap-3 justify-end mt-6">
              <button onClick={closeModal} className="px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                Cancelar
              </button>
              <button
        ...
        {modal === 'stock' && selected && (
          <ModalOverlay onClose={closeModal}>
            <h3 className="text-lg font-bold mb-1 text-slate-900 dark:text-slate-100">Ajustar Stock</h3>
            <p className="text-sm text-slate-500 mb-6">
              <span className="font-bold text-slate-700 dark:text-slate-300">{selected.nombre}</span>
              {' · '}Stock actual: <span className={cn('font-bold', selected.stock_actual <= selected.stock_minimo ? 'text-red-600' : 'text-slate-700 dark:text-slate-300')}>{selected.stock_actual}</span> unidades
            </p>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {stockForm.tipo === 'INGRESO' && stockForm.cantidad && (
                  <p className="text-xs text-green-600 mt-1">
                    Stock resultante: {selected.stock_actual + (Number(stockForm.cantidad) || 0)} unidades
                  </p>
                )}
                {stockForm.tipo === 'SALIDA' && stockForm.cantidad && (
                  <p className={cn('text-xs mt-1', Number(stockForm.cantidad) > selected.stock_actual ? 'text-red-600' : 'text-slate-500')}>
                    Stock resultante: {selected.stock_actual - (Number(stockForm.cantidad) || 0)} unidades
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStockForm((s) => ({ ...s, motivo: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-end mt-6">
              ...
              >
                {stockForm.tipo === 'INGRESO' ? <Icons.TrendingUp size={16} /> : <Icons.TrendingDown size={16} />}
                {saving ? 'Guardando...' : stockForm.tipo === 'INGRESO' ? 'Registrar Ingreso' : 'Registrar Salida'}
              </button>
            </div>
          </ModalOverlay>
        )}
