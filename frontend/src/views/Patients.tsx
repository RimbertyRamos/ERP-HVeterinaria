...
interface EspecieObj { id: string; nombre: string }
interface RazaObj { id: string; nombre: string; especie_id: string }
...
const speciesEmojis: Record<string, string> = {
  Perro: '🐕', Gato: '🐈', Ave: '🦜', Conejo: '🐇', Reptil: '🦎', Hamster: '🐹', Pez: '🐠'
};

const estadoColors: Record<string, string> = {
  ESPERA: 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800',
  EN_CURSO: 'bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800',
  COMPLETADA: 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800',
  CANCELADA: 'bg-slate-100 text-slate-400 border-slate-200 dark:bg-slate-800 dark:text-slate-500 dark:border-slate-700',
};
...
export const Patients: React.FC = () => {
  ...
  const [propietarioMode, setPropietarioMode] = useState<'nuevo' | 'existente'>('nuevo');
  const [propietarioSearch, setPropietarioSearch] = useState('');
  const [propietarioResults, setPropietarioResults] = useState<PropietarioResult[]>([]);
  const [selectedPropietarioId, setSelectedPropietarioId] = useState('');
  const [selectedPropietario, setSelectedPropietario] = useState<PropietarioResult | null>(null);

  const [form, setForm] = useState({
    nombre: '', especie_id: '', raza_id: '', sexo: 'M',
    fecha_nacimiento: '', peso_actual: '', alergias: '',
    propietario_nombre: '', propietario_email: '', propietario_telefono: '',
    propietario_ci: '', propietario_password: ''
  });

  const [soapForm, setSoapForm] = useState({
    motivo: '', diagnostico: '', tratamiento: '', hallazgos: '',
    peso: '', temp: '', fc: '', fr: ''
  });
  ...
  useEffect(() => {
    if (!form.especie_id) { setRazas([]); return; }
  ...
  const emitTicket = async (mascotaId: string, mascotaNombre: string) => {
    try {
      ...
      const serv = (servicios as any[]).find(s => s.nombre.toLowerCase().includes('consulta')) || servicios[0];
      if (!serv) throw new Error('No hay servicios configurados en el sistema');
      await api.createFicha({ mascota_id: mascotaId, servicio_id: serv.id, motivo: 'INGRESO MANUAL POR RECEPCIÓN' });
  ...
  const handleSearchPropietario = async () => {
    ...
    try {
      const results = await api.getUsuarios('CLIENTE', propietarioSearch.trim());
  ...
  const handleCreatePatient = async (e: React.FormEvent) => {
    ...
    if (propietarioMode === 'nuevo' && form.propietario_password.trim().length < 6) {
      toast.error('La contraseña del propietario debe tener al menos 6 caracteres');
      return;
    }
  ...
  const handleSaveSoap = async (e: React.FormEvent) => {
    ...
    try {
      const servicios = await api.getServicios();
      const servConsulta = (servicios as any[]).find(s => s.nombre.toLowerCase().includes('consulta')) || servicios[0];
      ...
      if (selectedExams.length > 0) {
        await Promise.all(selectedExams.map(examen_id =>
          api.createOrden({ ficha_id: ficha.id, examen_id, prioridad: 'NORMAL' })
        ));
      ...
      setSoapForm({ motivo: '', diagnostico: '', tratamiento: '', hallazgos: '', peso: '', temp: '', fc: '', fr: '' });
      setSelectedExams([]);
      setDetailTab('history');
  ...
  if (view === 'new') {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          <button onClick={() => setView('list')} className="text-slate-500 hover:text-primary transition-colors">
            <Icons.ArrowRight className="rotate-180" size={20} />
          </button>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Registrar Nuevo Paciente</h2>
        </div>
        ...
        <form onSubmit={handleCreatePatient} className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select required value={form.especie_id} onChange={e => setForm({ ...form, especie_id: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none">
              ...
              <div>
                ...
                <select value={form.raza_id} onChange={e => setForm({ ...form, raza_id: e.target.value })} disabled={razas.length === 0}
              ...
              <div>
                ...
                <input type="number" step="0.1" min="0" value={form.peso_actual} onChange={e => setForm({ ...form, peso_actual: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            ...
            <div className="flex rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 w-fit">
              <button type="button"
                onClick={() => { setPropietarioMode('nuevo'); setSelectedPropietarioId(''); setSelectedPropietario(null); }}
                className={cn('px-4 py-2 text-sm font-bold transition-colors',
              ...
              <button type="button"
                onClick={() => { setPropietarioMode('existente'); }}
                className={cn('px-4 py-2 text-sm font-bold transition-colors',
            ...
            {propietarioMode === 'nuevo' ? (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required value={form.propietario_nombre} onChange={e => setForm({ ...form, propietario_nombre: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Correo Electrónico *</label>
                  <input type="email" required value={form.propietario_email} onChange={e => setForm({ ...form, propietario_email: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Teléfono</label>
                  <input value={form.propietario_telefono} onChange={e => setForm({ ...form, propietario_telefono: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">CI / Carnet</label>
                  <input value={form.propietario_ci} onChange={e => setForm({ ...form, propietario_ci: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Contraseña de acceso *</label>
                  <input
                    type="password"
                    required
                    minLength={6}
                    value={form.propietario_password}
                    onChange={e => setForm({ ...form, propietario_password: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none"
                  />
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), handleSearchPropietario())}
                  ...
                  />
                  <button type="button" onClick={handleSearchPropietario}
                  ...
                  <div className="border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden">
                    {propietarioResults.map(p => (
                      <button key={p.id} type="button"
                        onClick={() => { setSelectedPropietarioId(p.id); setSelectedPropietario(p); setPropietarioResults([]); }}
                        className={cn(
                          ...
                          selectedPropietarioId === p.id ? 'bg-primary/10' : 'bg-white dark:bg-slate-900'
                        )}>
                        <p className="font-bold text-sm">{p.nombre}</p>
                        <p className="text-xs text-slate-500">{p.email}{p.ci ? ` · CI: ${p.ci}` : ''}</p>
                        {p._count && (
                          <p className="text-xs text-primary font-bold mt-0.5">
                            {p._count.mascotas} mascota{p._count.mascotas !== 1 ? 's' : ''} registrada{p._count.mascotas !== 1 ? 's' : ''}
                          </p>
                        )}
                      </button>
                    ))}
                ...
                {selectedPropietario && (
                  <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg flex items-center justify-between">
                    <div>
                      <p className="font-bold text-sm text-primary">{selectedPropietario.nombre}</p>
                      <p className="text-xs text-slate-500">{selectedPropietario.email}</p>
                      {selectedPropietario._count && (
                        <p className="text-xs text-slate-500 mt-0.5">
                          {selectedPropietario._count.mascotas} mascota{selectedPropietario._count.mascotas !== 1 ? 's' : ''} registrada{selectedPropietario._count.mascotas !== 1 ? 's' : ''}
                        </p>
                      )}
                    </div>
                    <button type="button" onClick={() => { setSelectedPropietarioId(''); setSelectedPropietario(null); }}
  ...
  if (view === 'detail' && selectedPatient) {
    const isInQueue = selectedPatient.fichas?.some(f => f.estado === 'ESPERA' || f.estado === 'EN_CURSO');
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 py-6">
          <div className="flex items-start justify-between gap-6">
            <div className="flex items-center gap-5">
              <button onClick={() => setView('list')} className="text-slate-500 hover:text-primary transition-colors flex-shrink-0">
                <Icons.ArrowRight className="rotate-180" size={20} />
              </button>
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-900 dark:text-white">{selectedPatient.nombre}</h2>
                <p className="text-sm text-slate-500">
                  {especieNombre(selectedPatient.especie)}
                  {razaNombre(selectedPatient.raza) ? ` · ${razaNombre(selectedPatient.raza)}` : ''}
                  {selectedPatient.sexo ? ` · ${selectedPatient.sexo === 'M' ? 'Macho' : 'Hembra'}` : ''}
                  {selectedPatient.peso_actual ? ` · ${selectedPatient.peso_actual} kg` : ''}
                </p>
              </div>
            </div>
            ...
            <div className="text-right">
              <div className="flex items-center gap-2 justify-end mb-1">
                ...
                <span className="font-bold text-slate-900 dark:text-white text-sm">{selectedPatient.propietario.nombre}</span>
              </div>
              {selectedPatient.propietario.email && (
                <p className="text-xs text-slate-500">{selectedPatient.propietario.email}</p>
              )}
              {selectedPatient.propietario.telefono && (
                <p className="text-xs text-slate-500">{selectedPatient.propietario.telefono}</p>
              )}
              <div className="mt-3">
                {isInQueue ? (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 text-xs font-black uppercase tracking-widest">
                    <span className="h-2 w-2 rounded-full bg-blue-500 animate-pulse" /> En cola de espera
                  </span>
                ) : (
                  <button
                    onClick={() => emitTicket(selectedPatient.id, selectedPatient.nombre)}
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 p-8">
          <AnimatePresence mode="wait">
            {detailTab === 'history' ? (
              <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-3xl mx-auto space-y-4">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6">
                  Historial de Atenciones · {selectedPatient.fichas?.length ?? 0} registros
                </h3>
                {(selectedPatient.fichas ?? []).length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-48 opacity-30 space-y-3">
                    ...
                    <p className="text-sm">Usa la pestaña "Nueva Atención" para registrar una consulta</p>
                  </div>
                ) : (
                  (selectedPatient.fichas ?? []).map(ficha => (
                    <div key={ficha.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <span className="text-xs font-black text-primary block">{formatDate(ficha.fecha_hora)}</span>
                          <h4 className="font-bold text-slate-900 dark:text-white">{ficha.servicio?.nombre ?? 'Consulta'}</h4>
                          <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                            {ficha.doctor && <span>Dr. {ficha.doctor.nombre}</span>}
                            {ficha.consultorio && <span>{ficha.consultorio.nombre}</span>}
                          </div>
                        </div>
                        <span className={cn('text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest border', estadoColors[ficha.estado] ?? estadoColors.CANCELADA)}>
                          {ficha.estado}
                        </span>
                      </div>

                      {ficha.soap && (ficha.soap.diagnostico || ficha.soap.tratamiento) && (
                        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-3">
                          {ficha.soap.diagnostico && (
                            <div>
                              ...
                              <p className="text-sm text-slate-700 dark:text-slate-300">{ficha.soap.diagnostico}</p>
                            </div>
                          )}
                          {ficha.soap.tratamiento && (
                            <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Tratamiento</p>
                              <p className="text-sm text-slate-700 dark:text-slate-300">{ficha.soap.tratamiento}</p>
                            </div>
                          )}
                        </div>
                      )}
              ...
              <motion.div key="soap" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-3xl mx-auto">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6">
                ...
                <form onSubmit={handleSaveSoap} className="space-y-5">
                  <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 space-y-4">
                    <div>
                      ...
                      <input required value={soapForm.motivo} onChange={e => setSoapForm({ ...soapForm, motivo: e.target.value })}
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      <div>
                        <label className="block text-xs font-bold mb-1 text-slate-500">Peso (kg)</label>
                        <input type="number" step="0.1" value={soapForm.peso} onChange={e => setSoapForm({ ...soapForm, peso: e.target.value })}
                      ...
                      <div>
                        <label className="block text-xs font-bold mb-1 text-slate-500">Temp (°C)</label>
                        <input type="number" step="0.1" value={soapForm.temp} onChange={e => setSoapForm({ ...soapForm, temp: e.target.value })}
                          className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary" />
                      </div>
                      <div>
                        <label className="block text-xs font-bold mb-1 text-slate-500">FC (lpm)</label>
                        <input type="number" value={soapForm.fc} onChange={e => setSoapForm({ ...soapForm, fc: e.target.value })}
                          className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary" />
                      </div>
                      <div>
                        <label className="block text-xs font-bold mb-1 text-slate-500">FR (rpm)</label>
                        <input type="number" value={soapForm.fr} onChange={e => setSoapForm({ ...soapForm, fr: e.target.value })}
                          className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary" />
                      </div>
                    </div>

                    <div>
                      <label className="block text-sm font-bold mb-1">Hallazgos del examen físico</label>
                      <textarea value={soapForm.hallazgos} onChange={e => setSoapForm({ ...soapForm, hallazgos: e.target.value })} rows={2}
                    ...
                    <div>
                      <label className="block text-sm font-bold mb-1">Diagnóstico</label>
                      <textarea value={soapForm.diagnostico} onChange={e => setSoapForm({ ...soapForm, diagnostico: e.target.value })} rows={2}
                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none resize-none" />
                    </div>

                    <div>
                      <label className="block text-sm font-bold mb-1">Tratamiento / Indicaciones</label>
                      <textarea value={soapForm.tratamiento} onChange={e => setSoapForm({ ...soapForm, tratamiento: e.target.value })} rows={3}
                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary outline-none resize-none" />
                    </div>
                    ...
                    {labOrders.length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {labOrders.map(exam => (
                            <label key={exam.id} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                              <input type="checkbox"
                                checked={selectedExams.includes(exam.id)}
                                onChange={e => setSelectedExams(prev =>
                                  e.target.checked ? [...prev, exam.id] : prev.filter(id => id !== exam.id)
                                )}
                  ...
                  <div className="flex justify-end gap-4">
                    ...
                    <button type="submit" disabled={isSavingSoap}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Pacientes</h2>
          <p className="text-slate-500 dark:text-slate-400">Listado general de pacientes y propietarios</p>
        </div>
      ...
      {loading ? (
        <div className="flex justify-center py-20 text-slate-400">Cargando pacientes...</div>
      ) : patients.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {patients.map(p => {
            const isInQueue = p.fichas?.some(f => f.estado === 'ESPERA' || f.estado === 'EN_CURSO');
            return (
              <div key={p.id}
                ...
                <div onClick={() => selectPatient(p.id)} className="cursor-pointer">
                  <div className="flex items-center gap-4 mb-3">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-black text-slate-900 dark:text-white truncate">{p.nombre}</h3>
                      <p className="text-xs text-slate-500 truncate">
                        {especieNombre(p.especie)}{razaNombre(p.raza) ? ` · ${razaNombre(p.raza)}` : ''}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-1">
                    <Icons.User size={11} className="text-slate-400 flex-shrink-0" />
                    <span className="font-medium truncate">{p.propietario.nombre}</span>
                  </div>
                  {p.propietario.telefono && (
                    <p className="text-xs text-slate-400 pl-4 truncate">{p.propietario.telefono}</p>
                  )}
                </div>
